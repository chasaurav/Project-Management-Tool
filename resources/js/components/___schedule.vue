...
export default {
  ...
  mounted() {
    // this.loadDetails();
    this.getDetailsForSchedule();
  },
  methods: {
    ...mapActions(["getDetailsForSchedule"]),
    /* loadDetails() {
      fetch("/schedule")
        .then((res) => res.json())
        .then((data) => {
    ...
    renderTask() {
      ...
      let projectStartDate = this.settings.startDate;
      // Default work start date/time each day
      let workStartDateTime = new Date(
        projectStartDate + ", " + this.settings.workTime
      );
      // Hours to work each day
      let workPerDay = this.settings.workHours;
